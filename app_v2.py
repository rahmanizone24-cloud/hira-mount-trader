...
nifty_html = (
    ...
    if nifty_change >= 0 else f'<a href="https://in.tradingview.com/chart/?symbol=NSE:NIFTY" target="_blank" class="index-badge idx-bear">NIFTY 50 ▼ {nifty_change}%</a>'
)
bank_html = (
    ...
    if bank_change >= 0 else f'<a href="https://in.tradingview.com/chart/?symbol=NSE:BANKNIFTY" target="_blank" class="index-badge idx-bear">BANK NIFTY ▼ {bank_change}%</a>'
)
sensex_html = (
    ...
    if sensex_change >= 0 else f'<a href="https://in.tradingview.com/chart/?symbol=BSE:SENSEX" target="_blank" class="index-badge idx-bear">SENSEX ▼ {sensex_change}%</a>'
)
...
# ---------------------------------------------------------
# 6. EXACT 8-CONDITION PRICE ACTION ENGINE
# ---------------------------------------------------------
...
def scan_single_fyers_stock(symbol):
    try:
        ...
        # A. Fetch Daily Candle for Previous Day High & Low (PDH & PDL)
        daily_data = {
        ...
        # B. Fetch 5-Min Intraday Candles
        ...
        # -------------------------------------------------------------
        # EXACT BULLISH LOGIC (ALL 8 CONDITIONS MATCH)
        # -------------------------------------------------------------
        bullish_match = (
            ...
            (c0['high'] > prev_day_high)                # Cond 7: [0] High > PDH Breakout
        )
        ...
        # -------------------------------------------------------------
        # EXACT BEARISH LOGIC (ALL 8 CONDITIONS MATCH)
        # -------------------------------------------------------------
        bearish_match = (
            ...
            (c0['low'] < prev_day_low)                  # Cond 7: [0] Low < PDL Breakdown
        )
...
with c1:
    top_g = (
        bullish_df.iloc[0] if not bullish_df.empty else
        {"symbol": "N/A", "change": 0, "time": "--:--", "qty": 0, "tv_url": "#"}
    )
    st.markdown(
        f"""
    <div class="stat-box">
...
with c2:
    top_l = (
        bearish_df.iloc[0] if not bearish_df.empty else
        {"symbol": "N/A", "change": 0, "time": "--:--", "qty": 0, "tv_url": "#"}
    )
    st.markdown(
        f"""
