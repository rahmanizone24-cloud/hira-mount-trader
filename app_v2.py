...
debug_stats = {
    ...
    "data_received": 0,
    "passed_range": 0,
    "passed_second_candle": 0,
    "passed_bias": 0,
    "reached_breakout_check": 0,
    "final_qualified": 0
}
...
def scan_single_stock(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not (first["low"] <= second["open"] <= first["high"] and
            ...
            return None
        debug_stats["passed_second_candle"] += 1
        ...
        if not (is_bullish or is_bearish):
            return None
        debug_stats["passed_bias"] += 1

        debug_stats["reached_breakout_check"] += 1
...
def run_scanner() -> Tuple[pd.DataFrame, dict]:
    global debug_stats
    debug_stats = {k: 0 for k in debug_stats}
...
# ----------------- DEBUG PANEL -----------------
st.markdown("---")
st.subheader("🔍 Debug Information (31 Jul Scan)")
st.write(f"""
