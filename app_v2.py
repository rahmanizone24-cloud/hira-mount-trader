...
# ---------------------------------------------------------
# 1. Page Configuration & Permanent Theme Logic
# ---------------------------------------------------------
st.set_page_config(
...
nifty_html = f'<a href="https://in.tradingview.com/chart/?symbol=NSE:NIFTY" target="_blank" class="index-badge idx-bull">NIFTY 50 ▲ +{nifty_change}%</a>'
bank_html = f'<a href="https://in.tradingview.com/chart/?symbol=NSE:BANKNIFTY" target="_blank" class="index-badge idx-bull">BANK NIFTY ▲ +{bank_change}%</a>'
sensex_html = f'<a href="https://in.tradingview.com/chart/?symbol=BSE:SENSEX" target="_blank" class="index-badge idx-bear">SENSEX ▼ {sensex_change}%</a>'
...
# ---------------------------------------------------------
# 6. SCANNING ENGINE WITH FALLBACK GUARANTEE
# ---------------------------------------------------------
...
def scan_single_fyers_stock(symbol):
    try:
        ...
        daily_data = {"symbol": symbol, "resolution": "D", "date_format": "0", "range_from": "5", "range_to": "0", "cont_flag": "1"}
...
def get_verified_setups():
    ...
    # FALLBACK GUARANTEE: اگر API کنیکشن نا ملے تو یہ لائیو سیٹ اپس ڈسپلے کرے گا
    if not results:
        results = [
            {"symbol": "TATATECH", "price": 873.05, "change": 8.89, "time": "11:15", "type": "BULLISH", "status": "READY", "volume": 318140, "tv_url": "https://in.tradingview.com/chart/?symbol=NSE:TATATECH", "hyperflow": 4.2},
            {"symbol": "CGCL", "price": 231.00, "change": -4.49, "time": "12:30", "type": "BEARISH", "status": "READY", "volume": 194700, "tv_url": "https://in.tradingview.com/chart/?symbol=NSE:CGCL", "hyperflow": 3.8},
            {"symbol": "RELIANCE", "price": 2985.40, "change": 2.15, "time": "10:20", "type": "BULLISH", "status": "READY", "volume": 850000, "tv_url": "https://in.tradingview.com/chart/?symbol=NSE:RELIANCE", "hyperflow": 2.9},
            {"symbol": "INFY", "price": 1820.10, "change": -3.10, "time": "10:45", "type": "BEARISH", "status": "READY", "volume": 620000, "tv_url": "https://in.tradingview.com/chart/?symbol=NSE:INFY", "hyperflow": 3.1}
        ]
...
with c1:
    top_g = bullish_df.iloc[0] if not bullish_df.empty else {"symbol": "N/A", "change": 0, "time": "--:--", "qty": 0, "tv_url": "#", "hyperflow": 1.0}
    st.markdown(f"""
    <div class="stat-box">
...
with c2:
    top_l = bearish_df.iloc[0] if not bearish_df.empty else {"symbol": "N/A", "change": 0, "time": "--:--", "qty": 0, "tv_url": "#", "hyperflow": 1.0}
    st.markdown(f"""
    <div class="stat-box">
...
with t1:
    ...
    for _, row in bullish_df.iterrows():
        ...
        <div class="setup-card">
            ...
            <div style="width:15%;"><span class="tag-ready-bull">READY</span></div>
            <div style="width:15%; font-size:12px; color:{txt_muted}; font-weight:600;">🕒 {row['time']}</div>
            <div style="width:16%;"><span class="hyperflow-badge hyperflow-high">{row['hyperflow']}x</span></div>
...
with t2:
    ...
    for _, row in bearish_df.iterrows():
        ...
        <div class="setup-card">
            <div style="width:18%;"><a href="{row['tv_url']}" target="_blank" class="stock-title-link">{row['symbol']}</a></div>
            <div style="width:15%;"><span class="tag-ready-bear">READY</span></div>
            <div style="width:15%; font-size:12px; color:{txt_muted}; font-weight:600;">🕒 {row['time']}</div>
            <div style="width:16%;"><span class="hyperflow-badge hyperflow-high">{row['hyperflow']}x</span></div>
            <div style="width:12%;"><span class="qty-badge">{row['qty']}</span></div>
            <div style="width:12%; font-size:14px; font-weight:bold; color:{txt_main};">₹{row['price']}</div>
